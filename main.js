...
// AUTO VIEW-ONCE SAVE
const { autoViewSaveCommand, handleAutoViewSave } = require('./commands/autoviewsave');
...
async function handleMessages(sock, update) {
    try {
        ...
        // Silent auto view-once save (runs on every message, checks internally)
        // Fire and forget — never blocks or crashes the main handler
        handleAutoViewSave(sock, message).catch(() => {});
        ...
        switch (cmd) {
            ...
            case 'autoviewsave': case 'avs': await autoViewSaveCommand(sock, chatId, message, args); break;
